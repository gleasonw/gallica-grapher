...
from request import Request, get_num_periods_in_range_for_grouping, SearchProgressStats
    ...
    def test_get_num_periods_in_range_for_grouping(self):
        self.assertEqual(
            get_num_periods_in_range_for_grouping(
                grouping="year", start="1900", end="1908"
            ),
            9,
        )
        self.assertEqual(
            get_num_periods_in_range_for_grouping(
                grouping="month", start="1900", end="1908"
            ),
            108,
        )
...
class TestSearchProgressStats(TestCase):
    ...
    def test_update_progress_all_search(self):
        ...
        # when
        stats.update_progress(elapsed_time=1, num_workers=1, xml=b"<test></test>")
        # then
        self.assertEqual(
            stats.to_dict(),
            {
    ...
    def test_update_response_time_all_search(self):
        ...
        # when
        stats.update_progress(elapsed_time=10, num_workers=2, xml=b"<test></test>")
        # then
        self.assertEqual(stats.average_response_time, 5.75)
        self.assertEqual(stats.estimate_seconds_to_completion, 46)
