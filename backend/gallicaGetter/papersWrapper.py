...
class PapersWrapper(GallicaWrapper):
    ...
    def get(
        self, arg_codes: List[str], stateHooks=None, get_all_results: bool = False
    ) -> List[PaperRecord]:
        if type(arg_codes) == str:
            arg_codes = [arg_codes]

        def build_sru_queries_for_codes() -> List[PaperQuery]:
            """Builds a Gallica query to fetch paper metadata for each code string"""
            sru_queries = []
        ...
        if not arg_codes and get_all_results:
            # Fetch all results, indexing by the number of papers on Gallica. Lengthy.
            queries = build_indexed_queries(
                [PaperQuery(start_index=0, num_records=1, endpoint=self.endpoint_url)],
                api=self.api,
            )
        elif not arg_codes and not get_all_results:
            raise ValueError("Must provide codes to get papers")
        else:
            if type(arg_codes) == str:
                codes = [arg_codes]
