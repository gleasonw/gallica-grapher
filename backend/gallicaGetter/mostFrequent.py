...
async def get_gallica_core(
    ...
    text_to_analyze = get_sample_text(
        root_gram, sample_size, start_date, end_date, onUpdateProgress
    )
    notable_words_in_distance = await get_associated_words(text_to_analyze, root_gram)
...
async def get_sample_text(
    ...
    async def get_text_for_codes(codes: List[str], target_word: str) -> str:
        text_wrapper = Context()
        text = ""
        text_records = await text_wrapper.get([(code, target_word) for code in codes])
