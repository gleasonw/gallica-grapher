...
class TestRealGets(unittest.TestCase):
    def test_get_volume_occurrences(self):
        ...
        self.assertEqual(len(records), 693)

    def test_get_volume_occurrences_multi_word_term(self):
        getter = gallicaGetter.connect("volume")
        records = getter.get(
            "organisation armée secrète", start_date="1900", end_date="2000"
        )
    ...
    def test_get_papers_wrapper(self):
        ...
        record = paper[0]
        self.assertEqual(record.code, "cb32895690j")
        self.assertEqual(record.publishing_years, [str(i) for i in range(1863, 1945)])
