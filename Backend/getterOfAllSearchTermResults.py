...
from Backend.dictionaryMaker import DictionaryMaker
from Backend.getterOfAllResultsFromPaper import *
...
class GallicaSearch:
	...
	def makeChunkedDictionary(self):
		...
		chunkSize = 20
		currentIndex = 0
		for i in range((len(self.newspaperDictionary) // 10) - 1):
			subDict = {}
...
class FullSearchWithinDictionary(GallicaSearch):
	...
	def createWorkersForFindingTotalResults(self):
		...
		totalIterations = ceil(len(self.newspaperDictionary) / 30)
