import { z } from "zod";
import { fetchSRU, fetchVolumeContext } from "./fetchContext";
...
export default async function Context({
  ...
  const result = searchPageState.safeParse(queryParams);
  if (!result.success) {
  ...
  function getArkImageFromParams(ark: string) {
    if (Object.keys(queryParams)?.includes(`arkPage${ark}`)) {
      return queryParams[`arkPage${ark}`];
    }
  }
